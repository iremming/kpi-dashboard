...
/**
 * Serverless function to fetch churn rate data with month-over-month change indicator
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // Query last 2 months of churn rate data to calculate change
    const query = `
      SELECT 
        date,
        churn_rate
    ...
    res.status(200).json({
      ...
      data: {
        ...
        change_percentage: Math.round(changePercentage * 100) / 100, // Round to 2 decimal places
