...
/**
 * Serverless function to fetch active users data with growth indicator
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // Query last 2 months of active users data to calculate growth
    const query = `
      SELECT 
        date,
        active_users
    ...
    if (result.rows.length < 2) {
      return res.status(200).json({
        success: true,
        data: {
          active_users: result.rows[0]?.active_users || 0,
          growth_percentage: 0
        }
      });
    }
