...
/**
 * Serverless function to fetch revenue by region data for the latest month
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // Query the latest month's revenue data for all 4 regions
    const query = `
      SELECT 
        region,
        revenue,
    ...
    if (result.rows.length === 0) {
      ...
      return res.status(200).json({
        success: true,
        data: [],
        message: 'No revenue by region data available'
      });
    }
    ...
    // Find the best performing region (highest revenue)
